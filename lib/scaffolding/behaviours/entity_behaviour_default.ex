...
defmodule Noizu.Scaffolding.EntityBehaviourDefault do
  ...
  def id_implementation(table, sref_prefix) do
    quote do
      ...
      def id(unquote(sref_prefix) <> identifier),
          do: __MODULE__.ref(identifier)
              |> __MODULE__.id()
  ...
  def sref_implementation(table, sref_prefix) do
    quote do
      ...
      def sref(identifier) when is_bitstring(identifier),
          do: ref(identifier)
              |> sref()
  ...
  def entity_txn_implementation(table, repo) do
    quote do
      ...
      def entity!(%__MODULE__{} = this, options), do: this
      def entity!(%@table{} = record, options), do: __MODULE__.expand(record.entity, options[:compression] || %{})
      def entity!(identifier, options),
          do: @repo.get!(
            __MODULE__.ref(identifier)
            |> __MODULE__.id(),
            Noizu.ElixirCore.CallingContext.internal(),
            options
          ) || __MODULE__.miss_cb(identifier, options)
    end # end quote
