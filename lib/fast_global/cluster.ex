...
defmodule Noizu.FastGlobal.V3.Cluster do
  ...
  def put(identifier, value, options) do
    ...
    cond do
      ...
      origin == nil -> :error
      true -> :rpc.cast(origin, Noizu.FastGlobal.V3.Cluster, :coordinate_put, [identifier, value, settings, options])
    end
  end
        ...
        Enum.map(update.pool,
          fn(n) ->
            if n == node() do
              ...
              :rpc.cast(n, Noizu.FastGlobal.V3.Cluster, :put, [identifier, update, options])
            end
          end)
      end)
    :ok
